...
Prefer an Azure Linux VM if Diab's student credits are available. Verify eligibility, available credit, and expected cost before selecting or provisioning a VM. Keep the Compose deployment portable to another paid Linux VM or a suitable spare computer if needed. Diab will purchase a project domain through Cloudflare later; the exact name is not selected yet. The deployment defaults below settle implementation choices; credit eligibility, domain purchase and any fallback spending remain external setup work.

This is the architecture to build, not evidence of a deployed service, redeemed credits, or a purchased domain. The Compose setup described below is the current local development scaffold.
...
- Initial validation target: an Ubuntu LTS x86-64 VM with 2 vCPUs, 4 GiB RAM and 30 GiB disk. These are trial sizing values, not a purchase or capacity guarantee; validate the real runtime limits and builds before selecting a paid host. Diab is the initial deployment/Access maintainer, subject to team review.
